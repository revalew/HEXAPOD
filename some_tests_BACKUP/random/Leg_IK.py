...
def coordinate_to_degrees(
    x, y
):  # function to convert coordinates to angles from the x-axis (0~360)
...
def IK(pos):  # pos = [x, y, z]
    ...
    theta1 = coordinate_to_degrees(
        x, y
    )  # this is the angle from x-axis in anticlockwise rotation
...
def IK_leg(
    pos, LegID=1
):  # modified version of IK(), and will take the offset angles of each leg into account
    x, y, z = pos[0], pos[1], pos[2]
    x += 0.00000001  # this is to avoid zero-division math error

    # specify the length of the leg components <- put your original values
    coxa = 8.5  # coxa length
    femur = 6.8  # femur length
    tibia = 12.5  # tibia length
    ...
    beta = acos(
        (femur**2 + c**2 - tibia**2) / (2 * femur * c)
    )  # calculate angle beta
    theta2 = beta + alpha  # find theta2
    theta3 = acos(
        (tibia**2 + femur**2 - c**2) / (2 * tibia * femur)
    )  # find theta3
    ...
    # theta3 += -90
    print(
        f"Leg = {1}, theta1 = {theta1}, theta2 = {degrees(theta2)}, theta3 = {degrees(theta3)}"
    )

    goal_position1 = int(((theta1 + 150) / 300.0) * 1023.0)
    goal_position2 = int(((degrees(theta2) + 150) / 300.0) * 1023.0)
    goal_position3 = int(((degrees(theta3) + 150) / 300.0) * 1023.0)

    return (goal_position1, goal_position2, goal_position3)
